...
def pipeline():
    ...
    molecule = builder.convert_to_pseudo(molecule, h_identifiers=["h", "H"])

    molecule = builder.remove_h(
        molecule, charge_distribution="heavy", h_identifiers=["h", "H"]
    )

    molecule = builder.assign_linkers(
        molecule, linker_atoms=[4, 35], linker_identifier="L"
    )
    ...
    molecule = reader.read_topo("PIM-1_fromTopo.tmp.lmps", pseudoatoms=True)
    ...
    molecule = builder.add_ff_params(
        molecule,
        gaff2,
        missing_params,
        pseudoatoms=True,
