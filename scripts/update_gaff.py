...
def pipeline():
    ...
    if gaff_loc.exists():
        reader.read_gaff2(gaff_loc, default_loc=False, out_json=pylift_loc / 'ff_data' / 'gaff.json')
    else:
        print('ANTECHAMBER not found on path, searching default location...')
        gaff_loc = Path(os.path.expanduser("~")) / 'amber24' / 'dat' / 'leap' / 'parm' / 'gaff.dat'

        if gaff_loc.exists():
            reader.read_gaff2('gaff.dat', default_loc=True, out_json=pylift_loc / 'ff_data' / 'gaff.json')
        else:
